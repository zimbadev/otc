...
function toggleContainerPages(containerWindow, pages)
    ...
    -- Se não existir o separador, mantenha o comportamento antigo por margens
    if not separator then
        scrollbar:setMarginTop(pages and 42 or 22)
        contentsPanel:setMarginTop(pages and 42 or 22)
        pagePanel:setVisible(pages)
        return
    end

    if pages then
        -- Ancoragem com separador presente
        ...
        contentsPanel:addAnchor(AnchorTop, 'pagePanel', AnchorBottom)
        ...
        -- Páginas ocultas: volta ao padrão
        scrollbar:breakAnchors()
    ...
    pagePanel:setVisible(pages)
    if separator then separator:setVisible(pages) end
end
